...
/**
 * Transformer: AGCO Dynamic Media / Scene7 images.
 * The source page serves images from Scene7 (s7d9.scene7.com/is/image/agco/...,
 * confirmed in migration-work/metadata.json .images.mapping). Scene7 <img> tags
 * carry IS/Image query params (e.g. ?fmt=png-alpha) that a raw <img src> would
 * not survive through the docx -> markdown round-trip.
 *
 * This transformer rewrites each DM <img> into an anchor so the URL round-trips
 * intact:
 *   - unlinked image  -> <a href="DM-URL">alt</a>
 *   - linked image    -> <a href="/page" title="DM-URL">alt</a>  (DM URL in title)
 * The companion client-side auto-block in scripts/scripts.js rebuilds these into
 * responsive <picture> elements at render time.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into block cells (cards/carousel/columns image cells); a
 * rewrite in beforeTransform would leave parsers with no <img> and emit empty
 * cells. See references/dm-scene7-transformer.md.
 *
 * Canonical helpers below are copied byte-identical from dm-scene7-helpers.js.
 */
...
export default function transform(hookName, element, payload) {
  ...
  element.querySelectorAll('img').forEach((img) => {
    ...
    // Empty alt is substituted with EMPTY_ALT_SENTINEL so authors see a visible
    // cell in Document view; the auto-block translates it back to alt="".
