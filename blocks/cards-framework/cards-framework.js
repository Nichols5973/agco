  ...
  [...block.children].forEach((row) => {
    ...
    [...li.children].forEach((div, index) => {
      if (index === 0) {
        ...
        div.className = 'cards-framework-card-image';
      } else if (index === 2 || index === 3) {
        // Third/fourth div - card style / CTA style configuration
        div.className = 'cards-framework-config';
        const p = div.querySelector('p');
        if (p) {
          p.style.display = 'none'; // Hide the configuration text
        }
      } else {
        // Second div (index 1) - content with button; any other divs
        div.className = 'cards-framework-card-body';
      }
    });
