...
function decorateImpl(block) {
  ...
  if (textDiv) textDiv.classList.add('hero-immersive-text');

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add('hero-immersive-video');
    ...
    video.className = 'hero-immersive-video-bg';
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = 'hero-immersive__badge';
