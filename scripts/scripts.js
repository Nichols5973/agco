...
// ---- Rendering ----
function appendSource(pictureEl, { type, srcset, media }) {
  const sourceEl = document.createElement('source');
  if (type) sourceEl.type = type;
  sourceEl.srcset = srcset;
  if (media) sourceEl.setAttribute('media', media);
  pictureEl.append(sourceEl);
}

function renderScene7Picture(src, alt) {
  const pictureEl = document.createElement('picture');
  DM_BREAKPOINTS.forEach((bp) => appendSource(pictureEl, {
    type: 'image/webp',
  ...
  imgEl.src = buildScene7Rendition(src, { width: 750, format: 'jpg' });
  imgEl.alt = alt;
  imgEl.loading = 'lazy';
...
function renderDmOpenApiPicture(src, alt) {
  ...
  const imgEl = document.createElement('img');
  imgEl.src = buildDmOpenApiRendition(src, { width: 750 });
  imgEl.alt = alt;
  imgEl.loading = 'lazy';
  pictureEl.append(imgEl);
  return pictureEl;
...
function buildDynamicMediaImages(main) {
  ...
  main.querySelectorAll('a').forEach((a) => {
    ...
    const pictureEl = detectDynamicMediaUrl(dmUrl) === 'scene7'
    ...
    if (isUnwrappableMarkdownParagraph(a)) {
      a.parentElement.replaceWith(pictureEl);
    } else {
